"""
Сборка одной "нарезки": вырезает сегмент [start, end] из видео (принимает
как URL, так и локальный путь — edits_main.py передаёт уже скачанный
локальный файл, см. game_trailer_source.download_trailer: seek по сети на
HLS-источнике Steam оказался ненадёжным), кадрирует под вертикальный
формат YouTube Shorts (1080x1920, crop-to-fill — как в примере-нарезке, без
чёрных полос), и заменяет звук на трек из trending_music.py (или оставляет
оригинальный звук исходника, если треков нет).
