...
Сборка вертикального видео (1080x1920, формат YouTube Shorts) из:
  - озвучки (mp3)
  - кадров персонажа (папка с PNG для цикла "разговора"; если папка пуста —
...
Персонаж (character_frames/frame_1..3.png) — неоновый чёрно-красный
призрак, сгенерированный ПОЛНОСТЬЮ ПРОГРАММНО (Pillow, без внешних
ассетов/шрифтов/лицензий стороннего происхождения) скриптом
generate_ghost_sprites.py в корне репозитория. Чтобы перегенерировать или
изменить кадры — правь этот скрипт и запускай его заново, он перезапишет
PNG в CHARACTER_FRAMES_DIR.
"""
...
CHARACTER_FRAMES_DIR = "character_frames"  # PNG-кадры talk-loop неонового призрака (см. generate_ghost_sprites.py)
...
# Призрак (аспект ~0.73) при ширине 560px ниже, чем прежний персонаж
# (аспект ~0.67), поэтому Y пересчитан так, чтобы нижний край силуэта
# остался на той же высоте, что и раньше (низ ~1580px) — персонаж по-прежнему
# визуально "стоит" на полу фона.
CHARACTER_Y = HEIGHT - 1102
...
def _character_clip(duration: float):
    """Зацикленная анимация персонажа (талк-луп призрака: рот закрыт ->
    приоткрыт -> открыт). Если папка с кадрами пуста — рисуется простая
    пульсирующая заглушка-кружок, чтобы пайплайн всё равно собрал видео."""
